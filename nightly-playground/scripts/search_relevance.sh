...
exe curl -s -X GET "$OPENSEARCH_SERVER_URL/_plugins/_search_relevance/experiments/$EX_POINTWISE"

echo
echo List experiments
exe curl -s -X GET "$OPENSEARCH_SERVER_URL/_plugins/_search_relevance/experiments" \
-H "Content-type: application/json" \
-d'{
     "sort": {
       "timestamp": {
...
exe curl -s -X GET "$OPENSEARCH_SERVER_URL/_plugins/_search_relevance/experiments/$EX_HO"
